        ...
        <a-modal v-model:visible="importVisible" @ok="handleImport" @cancel="afterImportClose"
            ...
            <div>
                <a-space direction="vertical" fill style="width: 100%">
                    ...
                    <a-space wrap>
                        <a-button @click="selectCurrentTables" :disabled="!currentTableKeys.length">选中当前列表</a-button>
                        <a-button @click="clearSelectedTables" :disabled="!selectedTables.length">取消选中</a-button>
                    </a-space>
                    <a-table row-key="tableName" :data="tableList" :bordered="{ cell: true }" :loading="tableLoading"
                        :pagination="false" :scroll="{ x: '100%', y: 400 }" v-model:selected-keys="selectedTables"
                        :row-selection="{
                            type: 'checkbox',
                            showCheckedAll: true,
                            onlyCurrent: false
...
const currentTableKeys = computed(() => tableList.value.map((item) => item.tableName));
...
const selectCurrentTables = () => {
    selectedTables.value = [...currentTableKeys.value];
};
